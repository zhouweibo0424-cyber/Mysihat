...
.tab-btn{
  ...
  background:#fff;
  color:#1f2328;              /* ✅ 明确指定未激活文字颜色（深灰） */
  padding:8px 14px;
  border-radius:999px;
  cursor:pointer;
  font-weight:600;
  line-height: 1;             /* 可选：让按钮更紧凑 */
}

.tab-btn:hover{
  background:#f6f8fa;         /* ✅ 悬停更明显 */
}
...
<?php if ($result): ?>
  <div class="box">
    ...
    <?php endif; ?>
    <?php if (!empty($aiExplain)): ?>
      <h3>AI Explanation</h3>
      <div class="note" style="white-space:pre-line"><?= h($aiExplain) ?></div>
    <?php endif; ?>
