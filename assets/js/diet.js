(function () {
  ...
  async function loadGoal() {
    const r = await fetchJSON("api/goal_get.php?user_id=" + encodeURIComponent(USER_ID) + "&_=" + Date.now(), { cache: "no-store" });
  ...
  async function saveGoal() {
    ...
    const r = await fetchJSON("api/goal_set.php", {
      method: "POST",
...
(function () {
  ...
  async function loadDay(dateStr) {
    const url = API_BASE + "/logs_get.php?user_id=" + encodeURIComponent(USER_ID) + "&date=" + encodeURIComponent(dateStr) + "&_=" + Date.now();
    const r = await fetchJSON(url, { cache: "no-store" });
  ...
  function renderLogs(rows) {
    ...
    for (const r of rows) {
      ...
      const qty = r.quantity || r.grams || r.qty || 1;
  ...
  async function addLog(dateStr, foodName, qty) {
    const payload = { user_id: USER_ID, date: dateStr, food_name: foodName, quantity: qty };
  ...
  async function deleteLog(id) {
    const r = await fetchJSON(API_BASE + "/logs_delete.php?id=" + encodeURIComponent(id) + "&_=" + Date.now(), { cache: "no-store" });
    if (!r.ok || !r.data || !r.data.success) {
